<template>
  <div>
    <quill-editor v-model="content"
                  ...
                  @change="onChange">
      <div v-if="fileUploadWay!=='base64'" id="quill-toolbar" slot="toolbar">
        ...
        <span class="ql-formats" v-if="fileUploadWay==='thirdParty'" id="uploadImgP">
...
//      CropUpload
    ...
    methods: {
      ...
      onFileChange(e){
        ...
        xhr.onload = function () {
          if (xhr.status === 200) {
            that.editor.insertEmbed(that.editor.getSelection().index, 'image', xhr.response.url)
          }
        }
      },
      //裁切上传成功 res根据上传接口值获取
      ...
      initUpload(){
        ...
        upload('uploadImg', 'uploadImgP',
          ...
          function (source) {
            console.log(source)
            that.editor.insertEmbed(that.editor.getSelection().index, 'image', source)
          })
      }
    },
    watch: {
